...
const monacoBasePath = path
  .dirname(import.meta.resolve("monaco-editor/package.json"))
  .replace(/^file:/, "");
    ...
    Icons({ compiler: "svelte" }),
    {
      ...
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          try {
            const url = req.url ?? "";
            if (!url.startsWith("/nu-on-web/assets/vs/")) return next();
            ...
            res.setHeader(
              "Content-Type",
              mime.lookup(monacoPath) || "application/octet-stream",
            );
